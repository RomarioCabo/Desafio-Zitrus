...
import br.com.zitrus.desafio.domain.Product_;
...
public class ProductRepositoryCustomImpl implements ProductRepositoryCustom {
    ...
    @Override
    public Page<ProductDto> filterProduct(ProductFilter filter, Pageable pageable, Direction direction) {
        ...
        if (filter.getIdCategory() != null) {
            predicates.add(criteriaBuilder.equal(root.get(Product_.CATEGORY).get(Category_.ID_CATEGORY), filter.getIdCategory()));
        }

        if (filter.getNameCategory() != null) {
            predicates.add(criteriaBuilder.equal(root.get(Product_.CATEGORY).get(Category_.NAME), filter.getNameCategory()));
        }
    ...
    private Order getOrdering(Pageable pageable, CriteriaBuilder criteriaBuilder, Root<Product> root, Direction direction) {
    ...
    private String getTableName(String sort) {
        switch (sort) {
            ...
            case Product_.SUPPLIER_VALUE:
                return Product_.SUPPLIER_VALUE;
            case Product_.QUANTITY_STOCK:
                return Product_.QUANTITY_STOCK;
            case Product_.ID_PRODUCT:
                return Product_.ID_PRODUCT;
            case Category_.ID_CATEGORY:
