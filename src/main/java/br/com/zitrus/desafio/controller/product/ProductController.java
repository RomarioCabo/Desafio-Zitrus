...
public class ProductController extends ApiHelper<ProductDto> {
  ...
  public ResponseEntity<ProductDto> save(@Valid @RequestBody ProductForm request) {
    ...
    return ResponseEntity.created(getUri(url + "api/v1/product", "id={id}", productDtoDto.getId()))
        .body(productDtoDto);
  }
  ...
  public ResponseEntity<ProductDto> update(
      @PathVariable Long idProduct, @Valid @RequestBody ProductForm request) {
  ...
  public ResponseEntity<List<ProductDto>> findCategories(
    ...
    Page<ProductDto> productsPage =
        productService.find(filters, page, linesPerPage, sortBy, direction);

    return ResponseEntity.ok()
        .headers(responseHeaders(productsPage))
        .body(productsPage.getContent());
  }

  @ApiResponses(
          value = {
                  @ApiResponse(code = 200, message = "OK", response = ProfitPerProductDto[].class),
                  @ApiResponse(code = 500, message = "Internal Error", response = StandardError.class)
          })
  @GetMapping(value = "/profit_per_product", produces = MediaType.APPLICATION_JSON_VALUE)
  public ResponseEntity<List<ProfitPerProductDto>> profitPerProduct() {
    return ResponseEntity.ok().body(stockMovementService.profitPerProduct());
  }
}
